...
export CFG_FILE=$CWD/config/config-file.json
export RMR_SEED_RT=$CWD/config/uta_rtg.rt
...
cd $CWD/alarm && RMR_SEED_RT=$CWD/config/uta_rtg_lib.rt go test . -v -coverprofile cover.out

# And for the Alarm Adapter
cd $CWD/adapter && go test -v -p 1 -coverprofile cover.out ./cmd/ -c -o ./adapter_test && ./adapter_test
