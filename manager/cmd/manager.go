...
func (a *AlarmManager) UpdateAlarmLists(newAlarm *alarm.AlarmMessage) {
	...
	if len(a.activeAlarms) >= a.maxActiveAlarms {
		...
		actAlarm := a.alarmClient.NewAlarm(alarm.ACTIVE_ALARM_COUNT_MORE_THAN_MAX_THRESHOLD, alarm.SeverityWarning, "clear alarms or raise threshold", "active alarms full")
	...
	if len(a.alarmHistory) >= a.maxAlarmHistory {
		...
		histAlarm := a.alarmClient.NewAlarm(alarm.ALARM_HISTORY_COUNT_MORE_THAN_MAX_THRESHOLD, alarm.SeverityWarning, "clear alarms or raise threshold", "alarm history full")
		histAlarmMessage := alarm.AlarmMessage{Alarm: histAlarm, AlarmAction: alarm.AlarmActionRaise, AlarmTime: (time.Now().UnixNano() / 1000)}
