...
func main() {
	...
	commando.
		...
		SetAction(func(args map[string]commando.ArgValue, flags map[string]commando.FlagValue) {
			displayAlarms(getAlarms(flags, "active"))
		})
	...
	commando.
		...
		AddFlag("port", "Alarm manager host address", commando.String, "8080").
		SetAction(func(args map[string]commando.ArgValue, flags map[string]commando.FlagValue) {
			displayAlarms(getAlarms(flags, "history"))
		})
...
func getAlarms(flags map[string]commando.FlagValue, action alarm.AlarmAction) (alarms []alarm.Alarm) {
...
func displayAlarms(alarms []alarm.Alarm) {
	t := table.NewWriter()
	t.SetOutputMirror(os.Stdout)
	t.AppendHeader(table.Row{"SP", "MOID", "APPID", "IINFO", "SEVERITY", "AAI"})

	for _, a := range alarms {
		t.AppendRows([]table.Row{
			{a.SpecificProblem, a.ManagedObjectId, a.ApplicationId, a.IdentifyingInfo, a.PerceivedSeverity, a.AdditionalInfo},
		})
	}
